<template>
  <el-row style="width:100%;height:100%;padding:0px" align="middle" justify="space-around" :gutter="40">
    <el-col :sm="24" :md="12" :xl="6">
      <el-tooltip class="box-item" effect="dark" content="下载PDF" placement="bottom">
        <el-button type="danger" class="btn" :disabled="paperInfo.open_access.is_oa == -1" @click="download">
          <el-icon color="#6b6b6b" size="large">
            <Download />
          </el-icon>
        </el-button>
      </el-tooltip>
    ...
    <el-col :sm="24" :md="12" :xl="6">
      <el-tooltip class="box-item" effect="dark" content="跳转到OpenAlex" placement="bottom">
        <el-button type="success" class="btn" @click="openAlex">
          <el-icon color="#6b6b6b" size="large">
            <View />
          </el-icon>
        </el-button>
    ...
    <el-col :sm="24" :md="12" :xl="6">
      <el-tooltip class="box-item" effect="dark" content="收藏" placement="bottom">
        <el-button type="success" class="btn" @click="star">
          <el-icon color="#6b6b6b" size="large">
            <Star v-if="amInList.length == 0" />
            <StarFilled v-else />
          </el-icon>
        </el-button>
      </el-tooltip>
    ...
    <el-col :sm="24" :md="12" :xl="6">
      <el-tooltip class="box-item" effect="dark" content="获取BiBTeX引用" placement="bottom">
        <el-button type="success" class="btn" @click="getBiBTeX(paperInfo), bibtexDialogVisible = true">
          <el-icon color="#6b6b6b" size="large">
            <Share />
          </el-icon>
        </el-button>
      ...
      <div v-if="starDialogVisible" class="dialog-window">
        ...
        <div v-if="collections.length != 0" class="dialog-content" style="white-space: pre-wrap;">
          <el-scrollbar max-height="400px">
            <el-checkbox class="cb" v-for="(collection, index) in collections" :key="index"
              @change="starChanged(collection)"
              :checked="amInList.find((col, idx, arr) => { return col.package_id == collection.id }) != null" size="large"
              border style="width:95%;margin-bottom:20px;border-raidus:0px">
              <el-tag type="info" effect="light" round>{{ collection.sum }}</el-tag>
              &nbsp;&nbsp;&nbsp;
              {{ collection.name }}
